...
export default function ResultsDisplay({ status, fileName, realResults, analysisId, vendorId }: ResultsDisplayProps) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.text(`Analyzed on ${formatDate(results.analyzedAt)}`, pageWidth / 2, yPos, { align: "center" })
      yPos += 15
      ...
      const protectionMeasures = [
        ...
        ["Proper Apron", results.protectionMeasures.properApron],
      ]
      ...
      pdf.setFont("helvetica", "bold")
      pdf.setTextColor(59, 130, 246)
      pdf.text("Hygiene Standards", margin, yPos)
      yPos += 8
      ...
      const hygieneStandards = [
        ["Clean Surfaces", results.hygieneStandards.surfaceCleanliness],
        ["Hair Covering", results.hygieneStandards.hairCovering],
      ]

      hygieneStandards.forEach(([key, value]) => {
        checkAddPage(15)
      ...
      pdf.setFont("helvetica", "bold")
      pdf.setTextColor(59, 130, 246)
      pdf.text("Hand Washing", margin, yPos)
      yPos += 8
      ...
      const handWashing = [
        ["Handwash Station", results.handWashing.handwashStation],
      ]

      handWashing.forEach(([key, value]) => {
        checkAddPage(15)
      ...
      const foodHandling = [
        ["Bare Hands", results.foodHandling.bareHands],
        ["Cross Contamination", results.foodHandling.crossContamination],
      ]
    ...
    if (notDetected) {
      ...
      switch (category) {
        case 'gloves':
          ...
          break
        case 'apron':
          displayValue = 'No apron detected'
          break
        case 'handwash':
          displayValue = 'No handwash station visible'
          break
        case 'pest':
          displayValue = 'No pest signs detected'
          break
        default:
    ...
    <div className="bg-white space-y-8 mt-8 relative pb-16">
      ...
      <div ref={reportRef} className="space-y-6">
        <div className="text-sm text-blue-800 mb-4">
          Analysis date: <span className="font-medium">{formatDate(results.analyzedAt)}</span>
        </div>
        ...
        <section>
          ...
          <div className="space-y-2">
            ...
            <MetricRow
              label="Proper Apron"
              value={results.protectionMeasures.properApron}
              notDetected={!results.detectionMetadata?.apronDetected && results.protectionMeasures.properApron === 0}
              category="apron"
            />
          </div>
        </section>
        ...
        <section>
          <h3 className="text-blue-500 font-medium mb-3">Hygiene Standards</h3>
          <div className="space-y-2">
            <MetricRow
              label="Clean Surfaces"
              value={results.hygieneStandards.surfaceCleanliness}
              notDetected={!results.detectionMetadata?.surfaceDetected && results.hygieneStandards.surfaceCleanliness === 0}
            />
            <MetricRow
              label="Hair Covering"
              value={results.hygieneStandards.hairCovering}
              notDetected={!results.detectionMetadata?.hairNetDetected && results.hygieneStandards.hairCovering === 0}
            />
          </div>
        </section>

        <section>
          <h3 className="text-blue-500 font-medium mb-3">Hand Washing</h3>
          <div className="space-y-2">
            <MetricRow
              label="Handwash Station"
              value={results.handWashing.handwashStation}
              notDetected={!results.detectionMetadata?.handwashDetected && results.handWashing.handwashStation === 0}
              category="handwash"
            />
          </div>
        </section>
        ...
        <section>
          ...
          <div className="space-y-2">
            <MetricRow
              label="Bare Hands"
              value={results.foodHandling.bareHands}
              notDetected={!results.detectionMetadata?.bareHandsDetected && results.foodHandling.bareHands >= 90}
            />
            <MetricRow
              label="Cross Contamination"
              value={results.foodHandling.crossContamination}
            />
          </div>
        </section>
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center text-xs text-gray-600 bg-white px-2 py-1 rounded">
                      ...
                      <span>{finding.timestamp === 'Throughout' ? 'Throughout video' : finding.timestamp}</span>
                    </div>
                  </div>
