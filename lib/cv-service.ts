...
export async function detectLabels(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const msg = errorBody.message || errorBody.error || `Vision API returned ${response.status}`
      console.error(`[v0] CV detection failed for frame ${frameNumber}: ${msg}`, errorBody)
      throw new Error(msg)
    }
    ...
    console.error(`[v0] CV detection failed for frame ${frameNumber}:`, error)
    throw error
  }
}
